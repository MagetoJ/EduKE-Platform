...
class PrinterService {
  ...
  /**
   * Print raw text summary (for reports)
   */
  async printSummary(text: string): Promise<{ success: boolean; message: string }> {
    const encoder = new TextEncoder();
    let commands: number[] = [];

    const addText = (t: string) => {
      commands.push(...Array.from(encoder.encode(t)));
    };
    ...
    // Initialize
    addCommand(`${ESC}@`);
    addCommand(`${ESC}a\x00`); // Left align
    ...
    try {
      ...
      return { 
        success: false, 
        message: error.message || 'Failed to print summary' 
      };
    }
  }

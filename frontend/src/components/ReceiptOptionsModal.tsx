...
import { Sale, Tenant } from '@/lib/api';
...
export function ReceiptOptionsModal({
  ...
  const handlePrintReceipt = async () => {
    ...
    try {
      const result = await printerService.smartPrint(sale, tenant, () => {
        // FALLBACK: If no printer is configured, show the browser dialog
        window.print();
      });
      ...
      if (!result.success && !result.usedFallback) {
        ...
        // If used fallback (browser dialog opened)
        onClose();
      }
    } catch (error: any) {
      console.error('Print error:', error);
      setStatusMessage('Failed to print');
    } finally {
      setPrinting(false);
    }
  };
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            {statusMessage && (
              <p className={`text-center text-sm font-medium ${statusMessage.startsWith('Error') ? 'text-red-600' : 'text-primary-600'}`}>
                {statusMessage}
              </p>
            )}

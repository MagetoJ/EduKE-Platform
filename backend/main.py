...
app = FastAPI(title="EduKE API", version="2.0.0")
...
async def startup_event():
    ...
    async with async_session_maker() as db:
        ...
        if not default_tenant:
            ...
            await db.commit()
            print(f"Default tenant created: demo.eduke.com")
...
async def login(
    ...
    if not login_data.subdomain:
        # If the user is a platform-wide superadmin, they can see ALL tenants
        if user.is_super_admin:
            ...
            # Add Platform Admin option first
            tenant_list.append({
                "tenant_id": 0, # Virtual ID for platform
            ...
            return {
                ...
                "tenants": tenant_list,
                "message": "System Manager Access: Select a school or proceed to Platform Admin"
            }
        ...
        # If Super Admin, add a special "Platform Administration" option
        if user.is_super_admin:
            tenant_list.append({
                "tenant_id": "platform",
                "tenant_name": "Platform Administration",
...
async def get_me(
    ...
    # Handle Super Admin Platform Access
    if current_user.is_super_admin and current_tenant is None:
        return {
            **current_user.__dict__,
            "is_super_admin": True,
            "role": UserRole.SUPER_ADMIN,
            "role_type": "platform_manager",
    ...
    if not membership:
        # If Super Admin is accessing a specific tenant they aren't a member of

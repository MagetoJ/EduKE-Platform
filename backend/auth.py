...
async def get_current_tenant_from_token(
    ...
    """Extract tenant from JWT token. Returns None for platform-wide super admins."""
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Could not validate credentials",
        headers={"WWW-Authenticate": "Bearer"},
    ...
    try:
        ...
        # If tenant_id is "platform", this is a super admin platform access
    ...
    # Ensure tenant_id is numeric for database lookup
    try:
...
async def get_current_tenant(
    ...
    current_tenant_from_token: Optional[Tenant] = Depends(get_current_tenant_from_token),
    db: AsyncSession = Depends(get_db)
) -> Optional[Tenant]:
    """
    Get current tenant and verify user has access to it.
    Returns None if user is a Super Admin accessing the platform panel.
    """
    # Super admins bypass all tenant checks if accessing platform panel
    if current_user.is_super_admin and current_tenant_from_token is None:
        return None

    if current_tenant_from_token is None:
         raise HTTPException(
